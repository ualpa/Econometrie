# Cluster 1 - Importanța caracteristicilor (prin algoritmul Random Forest):
# Social support                  0.237670
# Perceptions of corruption       0.208043
# Freedom to make life choices    0.201778
...
########################################################### REGRESIA MULTIPLA ########################################################### 
# Citirea datelor
ladderScore <- read.csv(paste0(directory, "Cluster1_updated.csv"))
...
#Ladder.score = 2.7151 + 1.6792*Social.support + 1.2517*Freedom.to.make.life.choices + Perceptions.of.corruption*2.6456
model_multiple <- lm(Ladder.score ~ Social.support+Freedom.to.make.life.choices+Perceptions.of.corruption,ladderScore)
summary(model_multiple)

# Intercept - Daca suportul social, Libertatea de alegere si Perceptia asupra coruptiei sunt 0, atunci variabila dependenta Ladder score este estimata a avea o valoare medie de 2.7151
# Social support - Daca suportul social creste cu o unitate, Ladder score-ul creste cu 1.6792 pastrand ceilalti termeni constanti
# Libertatea de alegere - Daca libertatea de alegere creste cu o unitate, Ladder score-ul creste cu 1.2517 pastrand ceilalti termeni constanti
# Peceptia asupra coruptiei - Daca perceptia asupra coruptiei crste cu o unitate, Ladder score-ul creste cu 2.6456 pastrand ceilalti termeni constanti
# Cele trei variabile sunt semnificative pentru modelul de regresie ales: Social support (pentru un nivel de semnificatie de 0.001), Libertatea de alegere (pentru un nivel de semnificatie de 0.01), Peceptia asupra coruptiei (pentru un nivel de semnificatie de 0.01)

# Bonitatea modelului (R-squared si R-squared ajustat) ----------------------------------------------------------------------------------------------------------------------------------------------------------------
# Regresie multipla cu 3 regresori
summary(model_multiple)
summary(model_multiple)$r.squared
# Aproximativ 39.05% din variatia variabilei dependente este explicata de variabilele independente din model
summary(model_multiple)$adj.r.squared
# Modelul explică aproximativ 36.06% din variația variabilei dependente
...
(coefficient <- coef(model_multiple)["Perceptions.of.corruption"])
...
#t statistic = 3.284362 > t critic = 1.999624 deci respingem ipoteza nula si acceptam ipoteza alternativa ca acest coeficient este semnificativ statistic pentru un nivel de incredere de 95%
...
# p-value =  1.114112e-06 este mult mai mic decat pragul de 0.05 deci confirma respingerea ipotezei nule
...
# p-value = 3.207e-09 este mult mai mic decat pragul de 0.05 deci respingem ipoteza nula ca datele urmeaza o distributie normala => va fi nevoie de o transformare a datelor pentru a le normaliza
...
# Histograma reziduurilor ------------------------------------------------------------------------------------------------------------http://127.0.0.1:18705/graphics/d29e8831-6caa-443b-82e0-a020617e05ae.png------------------------------------------------------------------------------------------------------------------------
...
# Histograma arata asimetrie la stanga deoarece observam valori extreme in jurul valorii de -1.5

#Asimetria ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
skewness(ladderScore$uhat)
# Valoarea negativa = 0.8615615 negative ne indica faptul ca distributia 
# noastra este centrata la stanga (asimetria)

# Boltirea (kurtosis) ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
kurtosis(ladderScore$uhat)
# kurtosis = 5.534938 => distributie leptocurtica
...
ladderScore_cook <- ladderScore[-c(65), ]
#reestimam modelul
model_multiple_C <- lm(Ladder.score ~ Social.support+Freedom.to.make.life.choices+Perceptions.of.corruption,ladderScore_cook)
summary(model_multiple_C)
ladderScore_cook %<>% mutate(uhat_cook = resid(model_multiple_C)) 
ols_plot_cooksd_bar(model_multiple_C) 


ladderScore_cook <- ladderScore_cook[-c(11,52,62,63,64), ]
#reestimam modelul
model_multiple_C <- lm(Ladder.score ~ Social.support+Freedom.to.make.life.choices+Perceptions.of.corruption,ladderScore_cook)
summary(model_multiple_C)
ladderScore_cook %<>% mutate(uhat_cook = resid(model_multiple_C)) 
...
ladderScore_cook <- ladderScore_cook[-c(3,55), ]
#reestimam modelul
model_multiple_C <- lm(Ladder.score ~ Social.support+Freedom.to.make.life.choices+Perceptions.of.corruption,ladderScore_cook)
summary(model_multiple_C)
ladderScore_cook %<>% mutate(uhat_cook = resid(model_multiple_C)) 
...
ladderScore_cook <- ladderScore_cook[-c(39), ]
#reestimam modelul
model_multiple_C <- lm(Ladder.score ~ Social.support+Freedom.to.make.life.choices+Perceptions.of.corruption,ladderScore_cook)
summary(model_multiple_C)
ladderScore_cook %<>% mutate(uhat_cook = resid(model_multiple_C)) 
...
jarque.bera.test(ladderScore_cook$uhat) # p-value=  0.4656 > 0.1 => reziduurile sunt normal distribuite
...
# Graficul reziduurilor fata de variabila independenta Perceptions of corruption
ggplot(data = ladderScore_cook, mapping = aes(x = Perceptions.of.corruption, y = uhat)) + 
  theme_bw() +
  geom_point() +
  geom_hline(yintercept = 0, col = 'red') + 
  labs(y = 'Reziduuri', x = 'Perceptia asupra coruptiei, Perceptions.of.corruption')
...
model_multiple3 <- lm(lLadder.score. ~ lSocial.support+Freedom.to.make.life.choices+Perceptions.of.corruption,ladderScore_cook)
...
# Regresie cu forma functionala log-log
model_multiple4 <- lm(lLadder.score. ~ lSocial.support+lFreedom.to.make.life.choices+Perceptions.of.corruption,ladderScore_cook)
summary(model_multiple4)
# Criteriul Akaike
aic <- AIC(model_multiple4)
cat("AIC (Akaike):", aic, "/n")

# Criteriul Schwarz 
bic <- BIC(model_multiple4)
cat("BIC (Schwarz):", bic, "/n")

# Regresie cu forma functionala log-log
model_multiple5 <- lm(lLadder.score. ~ lSocial.support+lFreedom.to.make.life.choices+lPerceptions.of.corruption,ladderScore_cook)
summary(model_multiple5)
# Criteriul Akaike
aic <- AIC(model_multiple5)
cat("AIC (Akaike):", aic, "/n")

# Criteriul Schwarz 
bic <- BIC(model_multiple5)
cat("BIC (Schwarz):", bic, "/n")

# Regresie cu forma functionala log-lin
model_multiple6 <- lm(lLadder.score. ~ Social.support+Freedom.to.make.life.choices+Perceptions.of.corruption,ladderScore_cook)
...
# Regresie cu forma functionala lin-log
model_multiple7 <- lm(Ladder.score ~ lSocial.support+lFreedom.to.make.life.choices+lPerceptions.of.corruption,ladderScore_cook)
summary(model_multiple7)
# Criteriul Akaike
aic <- AIC(model_multiple7)
cat("AIC (Akaike):", aic, "/n")

# Criteriul Schwarz 
bic <- BIC(model_multiple7)
cat("BIC (Schwarz):", bic, "/n")

#=> modelul 5 este cel mai bun

ladderScore_cook %<>% mutate(uhat1 = resid(model_multiple5))
# Graficul reziduurilor fata de variabila independenta lSocial.support
ggplot(data = ladderScore_cook, mapping = aes(x = lSocial.support, y = uhat1)) + 
  theme_bw() +
  geom_point() +
  geom_hline(yintercept = 0, col = 'red') + 
  labs(y = 'Reziduuri', x = 'Perceptia asupra suportului social logaritmat, lSocial.support')

# Graficul reziduurilor fata de valorile estimate de model
ladderScore_cook %<>% mutate(yhat = fitted(model_multiple5))
ggplot(data = ladderScore_cook, mapping = aes(x = yhat, y = uhat1)) + 
  theme_bw() +
  geom_point() +
  geom_hline(yintercept = 0, col = 'red') + 
...
ladderScore_cook %<>% mutate(Social.support = Social.support^2,
                             Freedom.to.make.life.choices = Freedom.to.make.life.choices^2,
                             Perceptions.of.corruption = Perceptions.of.corruption^2,
                             Freedom.to.make.life.choicesXPerceptions.of.corruption = Freedom.to.make.life.choices*Perceptions.of.corruption,
                             Freedom.to.make.life.choicesXSocial.support = Freedom.to.make.life.choices*Social.support,
                             Perceptions.of.corruptionXSocial.support = Perceptions.of.corruption*Social.support)

ladderScore_cook %<>% mutate(uhat1 = resid(model_multiple5),
                             yhat1 = fitted(model_multiple5), 
                             uhat1sq = uhat1^2, 
                             yhat1sq = yhat1^2)

# Testul Breusch-Pagan -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bptest(model_multiple5)

# Testul White ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
white_test(model_multiple5) 
...
# Autocorelarea ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
summary(model_multiple5)
acf(model_multiple5$residuals)

# Testul Durbin-Watson (ordinul 1) ---------------------------------------------------------------------------------------------------------------------------------------------------------------------

dwtest(model_multiple5) # p-value=3.221e-06 < 0.1 => rezidurile sunt autocorelate

bgtest(model_multiple5) # p-value < 0.1 
bgtest(model_multiple5, order = 2)  # p-value < 0.1 
bgtest(model_multiple5, order = 3)  # p-value < 0.1 
bgtest(model_multiple5, order = 4)  # p-value < 0.1 
...
ladder_data <- data.frame(ladderScore_cook, resid_mod1=model_multiple5$residuals)
...
model2 <- lm(Ladder.score ~ Social.support + Freedom.to.make.life.choices + Perceptions.of.corruption + lag1, data=ladder_data_2)
summary(model2)
model2 <- lm(Ladder.score ~ Social.support + Freedom.to.make.life.choices + lag1, data=ladder_data_2)
summary(model2)
...
# ladderScore = 5.32341 + 0.29536 * Social.support + 0.53251 * Freedom.to.make.life.choices + 11.37705 * lag1
...
var(ladder_data_2$Freedom.to.make.life.choices)
...
cor.test(ladder_data_2$Freedom.to.make.life.choices, model2$residuals) # p-value > 0.1 => nu sunt corelate
...
training.samples <- ladder_data_2$lLadder.score %>%
...
model_log_corrected_train <- lm(lLadder.score. ~ Social.support + Freedom.to.make.life.choices + lag1, data = train.data)
...
RMSE(y_pred, test.data$lLadder.score)

# MAE - Mean Absolute Error
MAE(y_pred, test.data$lLadder.score)

# MSE - Mean Squared Error
mse(y_pred, test.data$lLadder.score)

# MAPE - Mean Absolute Percentage Error
MAPE(y_pred, test.data$lLadder.score)

# Setul de date pentru prognoză 'out-of-sample'
out_of_sample <- data.frame(Social.support = c(1.3, 1.2, 1.4),
                            Freedom.to.make.life.choices = c(0.3, 0.5, 0.45),
...
# Pentru o persoana cu suport social = 1.3, libertate de alegere = 0.3 si lag1 = 0.03 scorul estimat de fericire este de 6.208437
# Pentru o persoana cu suport social = 1.2, libertate de alegere = 0.5 si lag1 = 0.01 scorul estimat de fericire este de 6.057862
# Pentru o persoana cu suport social = 1.4, libertate de alegere = 0.45 si lag1 = 0.02 scorul estimat de fericire este de 6.204078

# Regresie cu termeni de interactiune ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

summary(model2)
ladder_data_2 %<>% mutate(ladderscorehat2=fitted(model2))

#Plot pentru Freedom.to.make.life.choices
ggplot(ladder_data_2,aes(x=Freedom.to.make.life.choices))+
  theme_bw()+
  geom_point(aes(y=Ladder.score, col='Scorul fericirii'))+
  geom_point(aes(y=ladderscorehat2, col='Predictia liniara'))

#Plot pentru Social.support
ggplot(ladder_data_2,aes(x=Social.support))+
  theme_bw()+
  geom_point(aes(y=Ladder.score, col='Scorul fericirii'))+
  geom_point(aes(y=ladderscorehat2, col='Predictia liniara'))

model2.1 <-lm(Ladder.score ~ Social.support+ Freedom.to.make.life.choices + lag1+ Freedom.to.make.life.choicesXSocial.support, ladder_data_2)
summary(model2.1)

#Interactiunea dintre Freedom.to.make.life.choicesXSocial.support nu este semnificativa

ladder_data_2 %<>% mutate(lag1XSocial.support=lag1*Social.support,
                          lag1XFreedom.to.make.life.choices=lag1*Freedom.to.make.life.choices)
model2.2 <-lm(Ladder.score ~ Social.support+ Freedom.to.make.life.choices + lag1+ lag1XSocial.support, ladder_data_2)
summary(model2.2)

model2.3 <-lm(Ladder.score ~ Social.support+ Freedom.to.make.life.choices + lag1+ lag1XFreedom.to.make.life.choices, ladder_data_2)
summary(model2.3)

# Regresia cu variabile dummy --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

lm(Ladder.score ~ Region_Central.and.Eastern.Europe, ladder_data_2) %>% summary
#nu este semnificativ 


# Termeni de interactiune cu o alta variabila dummy -----------------------------------------------------------------------------------------------------------------------------------------------------------------
model2.dummy<-lm(Ladder.score ~ Social.support+ Freedom.to.make.life.choices + lag1+ Region_Central.and.Eastern.Europe, ladder_data_2)
summary(model2.dummy)

model2.dummy2<-lm(Ladder.score ~ Social.support+ Freedom.to.make.life.choices + lag1+ Region_Latin.America.and.Caribbean, ladder_data_2)
summary(model2.dummy2)

model2.dummy3<-lm(Ladder.score ~ Social.support+ Freedom.to.make.life.choices + lag1+ Region_Western.Europe, ladder_data_2)
summary(model2.dummy3) 
#  In acest caz variabila dummy Region_Western.Europe este semnificativa 
...
var(ladder_data_2$Region_Western.Europe)# toate valorile > 0 => ipoteza acceptata
...
cor.test(ladder_data_2$Freedom.to.make.life.choices, model2.dummy3$residuals) # p-value > 0.1 => nu sunt corelate
cor.test(ladder_data_2$lag1, model2.dummy3$residuals) # p-value > 0.1 => nu sunt corelate
cor.test(ladder_data_2$Region_Western.Europe, model2.dummy3$residuals)
# => ipoteza acceptata  
...
out_of_sample2 <- data.frame(Social.support = c(1.2),
                             Freedom.to.make.life.choices = c(0.5),
                             lag1 = c(0.02),
                             Region_Western.Europe=c(1))
...
# Pentru o persoana cu suport social = 1.3, libertate de alegere = 0.3, lag1 = 0.03  si care locuieste in Regiunea de Vest a Europei scorul estimat de fericire este de 6.353441
...
# Definim predictorii
x <- data.matrix(ladder_data_2[, c('Social.support', 'Freedom.to.make.life.choices', 'lag1', 'Region_Western.Europe')])
...
new <- matrix(c(0.8, 0.75, 0.2, 1), nrow=1, ncol=4) 
